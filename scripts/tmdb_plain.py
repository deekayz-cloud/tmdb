...
# Overflight-JSON-Export: zusaetzlich zu den lokalen Bildern eine JSON-Manifest-Datei
# schreiben, damit Projectivys Overflight-Plugin die Bilder per URL laden kann
# (statt ueber Projectivys lokalen Folder-Wallpaper-Mechanismus).
...
# Create directories to save the backgrounds (getrennt nach movies/tv fuer die JSON-Manifeste)
movies_dir = os.path.join(OUTPUT_ROOT, "images", "movies")
tv_dir = os.path.join(OUTPUT_ROOT, "images", "tv")
for background_dir in (movies_dir, tv_dir):
...
def process_image(image_url, title, is_movie, genre, year, rating, duration=None, seasons=None, category_dir=None):
    ...
    if response.status_code == 200:
        ...
        rel_path = os.path.relpath(filename, OUTPUT_ROOT)
        return {"title": title, "location": year, "url_1080p": f"{SITE_BASE_URL}/{rel_path}"}
    else:
...
for movie in trending_movies.get('results', []):
    ...
    if backdrop_path:
        ...
        # Process the image
        item = process_image(image_url, title, is_movie=True, genre=genre, year=year, rating=rating, duration=duration, category_dir=movies_dir)
        if item:
...
for tvshow in trending_tvshows.get('results', []):
    ...
    if backdrop_path:
        ...
        # Process the image
        item = process_image(image_url, title, is_movie=False, genre=genre, year=year, rating=rating, seasons=seasons, category_dir=tv_dir)
        if item:
...
with open(os.path.join(OUTPUT_ROOT, "tmdb_movies.json"), "w", encoding="utf-8") as f:
    json.dump(movie_items, f, ensure_ascii=False, indent=2)
with open(os.path.join(OUTPUT_ROOT, "tmdb_tv.json"), "w", encoding="utf-8") as f:
    json.dump(tv_items, f, ensure_ascii=False, indent=2)
print(f"Manifest written: {len(movie_items)} movies, {len(tv_items)} tv shows")
